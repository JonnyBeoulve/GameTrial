...
router.post('/new', function(req, res, next) {
  ...
  Customer.create(req.body)
    ...
    .catch(function(error) {
      res.render("new_customer", { errors: error.errors, first_name: req.body.first_name, 
        last_name: req.body.last_name, address: req.body.address, email: req.body.email, epic_id: req.body.epic_id, zip_code: req.body.zip_code });
    });
})
...
router.get('/:id', function(req, res, next) {
  ...
  Promise.all([foundCustomer, foundRental])
    .then(function(values) {
      res.render('details_customer', { customer: values[0], rentals: values[1], errors: errors });
  });
})
...
router.post('/:id', function(req, res, next) {
  ...
  Customer.update(req.body, {
    ...
    .catch(errors => {
      ...
      return res.redirect(`/customers/${req.params.id}?errors=${errorMessages}`);
  });
})
