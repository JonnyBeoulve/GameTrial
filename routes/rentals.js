...
router.get("/overdue_rentals", function(req, res, next) {
  Rental.findAll({
    ...
    include: [{ model: Game }, { model: Customer }]
  }).then(function(rentals) {
    res.render("all_rentals", {
      rentals,
      gameStatus: "/rentals/overdue_rentals",
...
router.get("/checked_rentals", function(req, res, next) {
  Rental.findAll({
    ...
    res.render("all_rentals", {
      rentals,
      gameStatus: "/rentals/checked_rentals",
      title: "Checked Out Games"
    });
  });
});
...
router.get("/new", function(req, res, next) {
  ...
  Promise.all([allGames, allCustomers]).then(function(values) {
    res.render("new_rental", {
      games: values[0],
      customers: values[1],
      todaysDate,
      returnDate
