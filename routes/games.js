...
router.post('/new', function(req, res, next) {
  ...
  Game.create(req.body)
    ...
    .catch(function(error) {
      res.render("new_game", { errors: error.errors, title: req.body.title, 
        genre: req.body.genre, developer: req.body.developer, first_published: req.body.first_published });
  });
})
...
router.get('/:id', function(req, res, next) {
  ...
  Promise.all([foundGame, foundRental])
    .then(function(values) {
      res.render('details_game', { game: values[0], rentals: values[1], errors: errors });
  });
})
...
router.post('/:id/return', function (req, res, next) {
  ...
  if ((!returned_on) || (returned_on.match (/[a-z]/i)) || (returned_on < todaysDate)) {
    ...
    .then(function (rentals) {
      res.render('return_game', { rental: rentals[0], customer: rentals[1], game: rentals[2], todaysDate, errors: error });
    });
  }
  else { 
